...
public class Map {
     ...
     * @param city2
     * @param end
     * @return ArrayList<Integer> return the road between 2 city ex : [8,2,5,7] : city 8 go to the city 7 passing by 2 and 5
     */
        ...
        while(road.get(size_road-1) != end){
            ...
            for (int j = 1;j <= size; j++){
                ...
                if(!memory.contains((j))){ // pas reboucler à l'infini
                    ...
                    else if (j == size && tab[i-1][j-1] == 0) { // there is no solution -> we have to remove the last element and go on ...
    ...
    public double distance(ArrayList<Integer> road){
        ...
        for (int i = 0; i< roadCity.size()-1;i++){
            ...
            distance += Math.sqrt(Math.pow(roadCity.get(i+1).getPos().getY() - roadCity.get(i).getPos().getY(),2) + (Math.pow(roadCity.get(i+1).getPos().getX() - roadCity.get(i).getPos().getX(),2)));
        }
     ...
     */
    public HashMap<String,Double> distanceMap(HashMap<String,ArrayList<Integer>> roadMap){ // hashmap(ville,distance,) pour avoir la cono de la ville
    ...
    public double[] costMap(HashMap<String,Double> distanceMap){
        ...
        for (CityWithPosition city : myMap.keySet()){ // for each city exept the producter : si 7 ville seulement 6 itérations
    ...
    public Results dayCitySimulate(HashMap<String,Double> distanceMap, String cityName){
        ...
        System.out.println(energyLoss[0]);
        for (CityWithPosition city : myMap.keySet()){
    ...
    public HashMap<String,double[]> lossDay(HashMap<String,Double> distanceMap){
        ...
        double[] energyLoss = getEnergy(costMap(distanceMap),SimType.DAY);
        System.out.println(energyLoss.length);
